...
async def test_button_handler_breaking_enable(tmp_db):
    ...
    with patch("newsdrop.bot.callbacks.set_breaking_news_preference", new_callable=AsyncMock) as mock_set:
        await callbacks.button_handler(update, context)
...
async def test_button_handler_unfollow_action(tmp_db):
    ...
    with patch("newsdrop.bot.callbacks.remove_followed_topic", new_callable=AsyncMock) as mock_remove:
...
async def test_button_handler_search_action(tmp_db):
    ...
    with patch("newsdrop.bot.callbacks.rate_limit_check", new_callable=AsyncMock, return_value=False), \
         patch("newsdrop.bot.callbacks.get_user_prefs", new_callable=AsyncMock, return_value={"country": "us", "category": "general"}), \
...
async def test_button_handler_search_rate_limited(tmp_db):
    ...
    with patch("newsdrop.bot.callbacks.rate_limit_check", new_callable=AsyncMock, return_value=True), \
         patch("newsdrop.bot.callbacks.search_news", new_callable=AsyncMock) as mock_search:
        await callbacks.button_handler(update, context)
...
async def test_button_handler_confirm_clear(tmp_db):
    ...
    update, query = _make_callback_query(
        f"confirm:clear:{user_id}:42", user_id=user_id
    )
    context = MagicMock()
...
async def test_button_handler_confirm_unfollow_all(tmp_db):
    ...
    user_id = 555
    update, query = _make_callback_query(
        f"confirm:unfollowall:{user_id}", user_id=user_id
    )
    context = MagicMock()

    with patch("newsdrop.bot.callbacks.clear_followed_topics", new_callable=AsyncMock) as mock_clear:
...
async def test_button_handler_confirm_wrong_user(tmp_db):
    ...
    update, query = _make_callback_query(
        "confirm:clear:999:42", user_id=111  # user_id mismatch
    )
    context = MagicMock()
...
async def test_button_handler_cancel(tmp_db):
    ...
    user_id = 555
    update, query = _make_callback_query(
        f"cancel:something:{user_id}", user_id=user_id
    )
    context = MagicMock()
